  ...
  <div class="message" v-if="isShow">
    ...
    <form action="">
      <div class="input">
        ...
        <p class="type">
          <input type="button" value="提交" @click="updating">
  ...
  export default{
    ...
    methods: {
        ...
        this.isShow = false;
          ...
          getData: function () {
            let _this = this;
            _this.$axios.post('/api/getUpdate',{
                id: _this._id
            },{
              ...
              .then(function (res) {
                  let time = res.data[0].playTime.split(':');
                _this.title = res.data[0].title;
                _this.name = res.data[0].name;
                _this.album = res.data[0].album;
                _this.playTime = time[0];
                _this.playTime1 = time[1];
                _this.type = res.data[0].type;
                _this.singUrl = res.data[0].singUrl;
                _this.composer = res.data[0].composer;
                _this.lyricist = res.data[0].lyricist;
                _this.arranger = res.data[0].arranger;
                _this.zanNum = res.data[0].zanNum;
                _this.playNum = res.data[0].playNum;
                _this.singPhoto = res.data[0].singPhoto;
                _this.createTime = res.data[0].createTime;
                _this.lyric = res.data[0].lyric;
              })
              .catch(function (err) {
                console.log(err)
              })
      ...
      sendMessage: function () {
        this.$emit('getMessage',this.success)
      }
    }
  }
</script>
